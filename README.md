...
项目目前尚未使用 Git tag 发布版本。以下记录依据仓库中的主要提交节点整理；`frontend/vue-app/package.json` 的版本号仅表示前端包版本，不代表整个平台版本。

### v1.2 · 2026-09-10

- 平台扩展为分析工作台、区域分析、模拟实验和模型推理四个独立页面。
- 新增 Region 独立 Runtime、研究区选择、加载阶段裁剪和二维 1 km 显示插值。
- 集成 Pisces-Simulator 与 Pisces-Ocean-Infer，并通过 Pisces-Hub 管理数据、模型和运行结果。
- 优化双窗口控制、分析工具栏、3D/2D 联动、刷新恢复和推理进度显示。
- Explorer 与 Region 使用相互独立的显示配置文件。

### 工作区重构 · 2026-07-28

- 统一单帧、时间序列和 A/B 对比工作区。
- 增加双序列配对、差值显示及比较指标。
- 拆分前端控制器与功能模块。
...
- 建立 Pisces-Explorer 项目。
- 提供基础 NetCDF 加载与海洋数据可视化能力。
